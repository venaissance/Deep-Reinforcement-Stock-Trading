...
def generate_price_state(stock_prices, end_index, window_size, volumes=None):
    """
    ...
    if start_index >= 0:
        period = stock_prices[start_index:end_index + 1] if volumes is None \
            else np.multiply(stock_prices[start_index:end_index + 1], volumes[start_index:end_index + 1])
    else:  # if end_index cannot suffice window_size, pad with prices on start_index
        period = -start_index * [stock_prices[0]] + stock_prices[0:end_index + 1] if volumes is None \
            else np.multiply(-start_index * [stock_prices[0]] + stock_prices[0:end_index + 1],
                             -start_index * [volumes[0]] + volumes[0:end_index + 1])
...
def generate_combined_state(end_index, window_size, stock_prices, balance, num_holding, volumes=None):
    """
    return a state representation, defined as
    ...
    """
    prince_state = generate_price_state(stock_prices, end_index, window_size, volumes)
...
def evaluate_portfolio_performance(agent, logger):
    ...
    logger.info('Sharpe Ratio adjusted with Treasury bond daily return: {:.3f}'.format(
        sharpe_ratio(np.array(agent.return_rates)), risk_free=treasury_bond_daily_return_rate()))
...
def plot_portfolio_performance_comparison(stock_name, agent):
    ...
    plt.plot(dates, agent.portfolio_values, color='green',
             label='{} Total Return: ${:.2f}'.format(agent.model_type, agent_return))
    plt.plot(dates, buy_and_hold_portfolio_values, color='blue',
    ...
    if '^GSPC' not in stock_name:
        dates, GSPC_buy_and_hold_portfolio_values, GSPC_buy_and_hold_return = buy_and_hold_benchmark('^GSPC_2018',
                                                                                                     agent)
        plt.plot(dates, GSPC_buy_and_hold_portfolio_values, color='red',
                 label='S&P 500 2018 Buy and Hold Total Return: ${:.2f}'.format(GSPC_buy_and_hold_return))
...
def plot_all(stock_name, agent):
    ...
    ax[1].plot(dates, agent.portfolio_values, color='green',
               label='{} Total Return: ${:.2f}'.format(agent.model_type, agent_return))
    ...
    if '^GSPC' not in stock_name:
        dates, GSPC_buy_and_hold_portfolio_values, GSPC_buy_and_hold_return = buy_and_hold_benchmark('^GSPC_2018',
                                                                                                     agent)
        ax[1].plot(dates, GSPC_buy_and_hold_portfolio_values, color='red',
                   label='S&P 500 2018 Buy and Hold Total Return: ${:.2f}'.format(GSPC_buy_and_hold_return))
...
def plot_portfolio_returns_across_episodes(model_name, returns_across_episodes, stock_name, num_episode,
                                           volume_considered=None):
    len_episodes = len(returns_across_episodes)
    plt.figure(figsize=(15, 5), dpi=100)
    plt.title(
        '{} Portfolio Returns {} {} Episodes'.format(stock_name, '(With Volume)' if volume_considered else '(Default)',
                                                     num_episode))
